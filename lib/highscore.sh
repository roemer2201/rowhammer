...
#   "rows|lines|level|name|date|gold|silver|time|rowhammers", sorted by rows (the
...
#   trailing rowhammers field (0.25.0) the round's number of four-row
#   clears; lines that do not match the nine-field format simply fail
...
#   squares, rowhammers, play time (MM:SS) and date.
...
# In-memory list: one
# "rows|lines|level|name|date|gold|silver|time|rowhammers" string per
# element, sorted by rows descending. HS_LAST_RANK is the rank the most
# recently added round reached (1-based, 0 = not on the list). The time
# field is the round's play time in whole seconds, the trailing field
# its number of four-row clears.
HS_ENTRIES=()
HS_LAST_RANK=0
...
# game writes round-trips unchanged. All nine fields are mandatory:
# the scoring rebuild dropped the old leading score field, play time and
# rowhammers were appended last, and per the no-backward-compatibility
# rule old-format lines are simply invalid.
HS_LINE_RE='^[0-9]+\|[0-9]+\|[0-9]+\|[A-Za-z0-9_ -]{1,16}\|[0-9]{4}-[0-9]{2}-[0-9]{2}\|[0-9]+\|[0-9]+\|[0-9]+\|[0-9]+$'
...
# highscore_add ROWS LINES LEVEL NAME GOLD SILVER TIME ROWHAMMERS
...
highscore_add() {
    ...
    entry="${rows}|${lines}|${level}|${name}|$(date +%Y-%m-%d)|${gold}|${silver}|${time}|${hammers}"
    if [ "${#HS_ENTRIES[@]}" -gt 0 ]; then
...
# rowhammers ("RH", four rows at once), the round's play time (MM:SS)
# and the date. Lines and level stay stored
...
# indent) fits the 48-column minimum exactly, which is why every new
# column is paid for by an existing one: the name is capped at 6
# characters (longer names are truncated for display only) and "Silber"
# shortened to "Silb" to make room for the RH column (0.25.0), just as
# the name went from 13 to 8 when the Zeit column arrived (0.17.0).
highscore_screen() {
    local -a body=()
    local i line hs_rows hs_name hs_date hs_gold hs_silver hs_time
    local hs_hammers mmss
    if [ "${#HS_ENTRIES[@]}" -eq 0 ]; then
        body+=("Noch keine Eintraege.")
        body+=("")
        body+=("Spiele eine Runde, um dich einzutragen.")
    else
        printf -v line '%2s %-6s %5s %4s %4s %3s %5s %10s' \
            "Nr" "Name" "Rows" "Gold" "Silb" "RH" "Zeit" "Datum"
        body+=("${line}")
        for i in "${!HS_ENTRIES[@]}"; do
            IFS='|' read -r hs_rows _ _ hs_name hs_date hs_gold hs_silver \
                hs_time hs_hammers <<< "${HS_ENTRIES[i]}"
            printf -v mmss '%02d:%02d' \
                "$(( hs_time / 60 ))" "$(( hs_time % 60 ))"
            printf -v line '%2d %-6.6s %5d %4d %4d %3d %5s %10s' \
                "$(( i + 1 ))" "${hs_name}" "${hs_rows}" \
                "${hs_gold}" "${hs_silver}" "${hs_hammers}" "${mmss}" \
                "${hs_date}"
            body+=("${line}")
        done
    fi
