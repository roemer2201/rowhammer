...
#   the round takes in the list of its mode, asked from
...
menu_help_body() {
    ...
    case "${page}" in
        ...
        8)
            ...
            HELP_BODY+=("${line}")
            # Horizontal arrows pick the versus seat; vertical arrows
            # and "-"/"+" control speed in every demo (CLAUDE.md 5.20).
...
#
# Called from record_round, and only for a round that really takes a
# place in one of the lists: a round nobody files anywhere has no name to
# ask for. Everything else the round feeds - wonder progress, statistics
# - is nameless anyway.
#
# Above the input line the prompt shows what the round achieved,
# including the place it takes in the list of its mode (0.24.0, user
...
prompt_round_name() {
    ...
    # returned the name it is filed under (see record_round).
    # CHANGE 2026-08-06 (user request): there is no "missed the list"
    # case here any more. Since 1.0.1 round_is_ranked asks the same
    # preview before it lets this prompt run at all, so a round without a
    # place is never asked for a name - it goes straight to the
    # end-of-round box (see rowhammer.sh).
    round_rank_preview
